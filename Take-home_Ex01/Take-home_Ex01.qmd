...
- Master Plan 2019 Subzone Boundary (No Sea) [from Data.gov.sg](data.gov.sg)
...
```{r}
# grabposisi_sf <- st_as_sf(grabposisi, coords=c("rawlat", "rawlng"), crs=4326) |> st_transform(crs=3414)
grabposisi_sf <- st_as_sf(grabposisi, coords=c('rawlat', 'rawlng'), crs=4326)
st_crs(grabposisi_sf)

```
Now we can check how it looks like by plotting it
...
```{r}
mpsz2019_sf <- st_read(dsn='data/Geospatial/MasterPlan2019SubzoneBoundaryNoSeaGEOJSON.geojson')
st_crs(mpsz2019_sf)
```
```{r}
plot(mpsz2019_sf)
...
```{r}
road_sf <- st_read('data/Geospatial/malaysia-singapore-brunei-latest-free.shp', layer='gis_osm_roads_free_1')
```
```{r}
st_crs(road_sf)
```
...
tm_shape(mpsz2019_sf) + tm_polygons() +
  tm_shape(road_sf) + tm_polygons() +
  tm_shape(grabposisi_sf) + tm_dots()
```
